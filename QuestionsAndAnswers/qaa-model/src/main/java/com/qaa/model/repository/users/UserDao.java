...
public interface UserDao extends JpaRepository<UserVo, Long> {
  ...
  void update(@Param("id") Long id, @Param("name") String name, @Param("surname") String surname, @Param("description") String description,
              @Param("age") int age);
  ...
  @Query("select u from UserVo u where (u.username=:username or u.mail=:email) and u.state=1")
  UserVo verify(@Param("username") String username, @Param("email") String email);

  @Query("select case when count(u)>0 then true else false end from UserVo u where u.username=:username")
  boolean existUsername(@Param("username") String username);
  
  @Query("select case when count(u)>0 then true else false end from UserVo u where  u.mail=:email")
  boolean existMail(@Param("email") String email);
